...
**Phase 1 implementation:** For the phase 1 implementation, we used native mechanisms provided by the browser to ensure that the required sandboxing properties were satisfied. The core runtime components execute inside an iFrame, which implements the function of the core sandbox. The iFrame isolates the code of the core runtime from the main window in which the javascript code of the application is executed. In other words, this isolation mechanism prevents applications from having direct access to the memory address space of the core runtime. Communication between application and core runtime is possible only through a single and well defined entrypoint that allows them to exchange messages: method postMessage(). Hyperties and protoStub execute as independent Web Workers. Web Workers effectively isolate their internal states from each other and from the core runtime. The postMessage() method constitutes the only communication bridge between the sandboxed components. Based on this analysis, we conclude that the phase 1 implementation prototype satisfies the security properties that were specified for the browser runtime architecture.
...
**Phase 1 implementation:** In the phase 1 implementation, we use Crosswalk in order to support standalone applications. Crosswalk is an HTML application runtime that allows us to execute the reTHINK runtime in Android and iOS devices as native mobile applications without the need to install a full-blown browser. Instead, mobile applications can be bundled with Crosswalk webviews which incorporates a Chromium-based runtime. A major benefit of this approach is that the phase 1 implementation for the browser platform can be reused as a software component to be deployed on Crosswalk webviews for the standalone platform. Consequently, standalone applications will satisfy the same security properties as full-blown browser applications.
